...
- **☁️ 账号系统与云端同步 (Cloud Sync)**: 通过 Firebase 强无缝集成，支持快速 Google 账号登录。可随时在本地/云端模式之间任意切换。在云端模式下，您的所有设置与对话记录都会自动跨端漫游，安全可靠。
- **🔄 离线请求托管 (Offline Request Delegation)**: 即使在等待长时间的AI绘图或者文本生成过程中不小心关闭了页面，Node.js 后端也会继续代您等待生成完毕，并悄悄为您同步至云端聊天记录中。当您下次使用任意设备登录账号时，那些完成的图片与对话都在那里！
- **🔒 安全与隐私**: API 密钥仅保存在您的本地浏览器中 (Local Storage)，不会同步到云端。
